        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <Section title="Repair time & expenditure" description="Average days to repair and monthly spend (₹ crore).">
            <ResponsiveContainer width="100%" height={260}>
              <LineChart
                data={
                  Array.isArray(work.data.repairTime)
                    ? work.data.repairTime.map((item, i) => ({
                        ...item,
                        crore: Array.isArray(work.data.expenditure)
                          ? work.data.expenditure[i]?.crore ?? 0
                          : 0,
                      }))
                    : []
                }
