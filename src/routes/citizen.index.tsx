...
import { CITIZEN_ID } from "@/utils/session";
...
function CitizenHome() {
  ...
  if (complaints.isError || assets.isError)
    return <ErrorState onRetry={() => void complaints.refetch()} />;

  const mine = (complaints.data ?? []).filter((c) => c.citizenId === CITIZEN_ID);
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-xl bg-navy px-4 py-5 text-navy-foreground">
        ...
        <h1 className="mt-1 text-xl font-semibold">Help keep your roads safe</h1>
        <p className="mt-1 text-[13px] text-navy-foreground/80">
          Every report is analysed by AI, scored for risk and tracked to verified repair.
        </p>
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatTile icon={ClipboardList} label="My reports" value={mine.length} />
        <StatTile icon={ShieldCheck} label="Resolved" value={resolved} />
        <StatTile
          icon={TriangleAlert}
          label="In progress"
          value={mine.length - resolved}
        />
      </div>
      ...
      >
        {complaints.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            You haven't reported anything yet. Your first report takes under a minute.
          </p>
        ) : (
          mine.slice(0, 4).map((c) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-foreground">{c.issueType}</p>
      ...
      <Section title="Infrastructure near you" description="Live condition of assets in your district.">
